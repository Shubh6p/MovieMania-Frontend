    ...
    authFetch("https://moviemania-backend-31wk.onrender.com/upload-poster", {
      ...
      .then(resp => {
        if (resp.startsWith("success:")) {
          ...
          updatedMovie.poster = `https://moviemania-backend-31wk.onrender.com/images/${fileName}`;
        }
...
function confirmDeleteMovie() {
  ...
  const deletedBy = sessionStorage.getItem("admin_user");
  ...
  authFetch("https://moviemania-backend-31wk.onrender.com/upload-poster", {
    ...
    .then((response) => {
      if (response.startsWith("success:")) {
        ...
        const uploadedPosterPath = `https://moviemania-backend-31wk.onrender.com/images/${fileName}`;
        ...
        authFetch("https://moviemania-backend-31wk.onrender.com/save/movies", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        alert(" Poster upload failed: " + response);
      }
    });
}
...
function saveSeriesToServer() {
  ...
  data[seriesKey].addedBy = getAdminUsername();

  authFetch("https://moviemania-backend-31wk.onrender.com/save/series", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  })
...
function login() {
  ...
  fetch("https://moviemania-backend-31wk.onrender.com/api/login", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
